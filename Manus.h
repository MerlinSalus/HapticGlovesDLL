...
extern "C" {
	...
	*/
	MANUS_API manus_ret_t ManusGetBatteryLevel(manus_session_t session, device_type_t device_type, uint8_t* out_percentage);
	/*! Get the current battery level in percent.
	*/
	MANUS_API manus_ret_t ManusGetBatteryLevel_id(manus_session_t session, uint32_t dongle_id, device_type_t device_type, uint8_t* out_percentage);
	...
	*/
	MANUS_API manus_ret_t ManusGetSignalStrength(manus_session_t session, device_type_t device_type, int16_t* out_rssi);
	/*! Get the signal strength.
	*/
	MANUS_API manus_ret_t ManusGetSignalStrength_id(manus_session_t session, uint32_t dongle_id, device_type_t device_type, int16_t* out_rssi);
	...
	*/
	MANUS_API manus_ret_t ManusGetHand(manus_session_t session, device_type_t device_type, manus_hand_t* out_hand);
	/*! Get the data from the glove and convert it to a skeletal model. The skeletal model is scaled
	*   according to the user profile.
	*
	*  \see ManusGetProfile
	...
	*  \param timeout The time in milliseconds to block waiting on a new packet.
	*/
	MANUS_API manus_ret_t ManusGetHand_id(manus_session_t session, uint32_t dongle_id, device_type_t device_type, manus_hand_t* out_hand);
	...
	*  \param timeout The time in milliseconds to block waiting on a new packet.
	*/
	MANUS_API manus_ret_t ManusGetHandRaw(manus_session_t session, device_type_t device_type, manus_hand_raw_t* out_hand);
	/*! Get the raw data from the glove. This does not include any of the skeletal model data. If you're also
	*   interested in the skeletal model you should call ManusGetHand() instead.
	*
	*  \see ManusGetHand
	*
	*  \param dongle_id The dongle ID of the dongle that you want to get the data from.
	*  \param device_type The device type to query.
	*  \param out_hand The output struct.
	*  \param timeout The time in milliseconds to block waiting on a new packet.
	*/
	MANUS_API manus_ret_t ManusGetHandRaw_id(manus_session_t session, uint32_t dongle_id, device_type_t device_type, manus_hand_raw_t* out_hand);
